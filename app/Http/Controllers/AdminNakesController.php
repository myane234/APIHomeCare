    ...
    public function registerViaAdmin(Request $request)
{
    ...
    $validate = $request->validate([
        ...
        'email.required'                 => 'Email user target wajib diisi.',
        'email.email'                    => 'Format email tidak valid.',
        'email.exists'                   => 'Email user tidak ditemukan di sistem.',
    ...
    $pasien = Pasien::whereHas('user', function ($query) use ($validate) {
    ...
    if (!$pasien) {
        return response()->json([
            'success' => false,
            'message' => 'Gagal mendaftarkan Nakes. User dengan email tersebut belum terdaftar sebagai Pasien.'
        ], 422);
    }

    $targetUserId = $pasien->id_user;
